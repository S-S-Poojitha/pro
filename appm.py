...
import cv2  # For computing color histograms
...
# Function to extract features with color histogram
def extract_features_with_color(image_path):
    # Extract high-level features
    high_level_features = extract_features(image_path)
    
    # Compute color histogram features
    color_histogram = compute_color_histogram(image_path)
    
    # Combine features
    combined_features = np.concatenate((high_level_features.flatten(), color_histogram))
...
def load_dataset_and_extract_features(dataset_path):
    ...
    for image_path in image_paths:
        features = extract_features_with_color(image_path)
...
if uploaded_file is not None:
    ...
    uploaded_image_features = extract_features_with_color(uploaded_image_path)
