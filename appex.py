...
def assign_label(uploaded_image_features):
    try:
        ...
        st.error(f"Error in assigning label: {e}")
        raise
...
def load_images_from_directory(directory):
    try:
        ...
        st.error(f"Error in loading images from directory: {e}")
        raise
...
def compute_color_histogram(image_path):
    try:
        ...
        st.error(f"Error in computing color histogram: {e}")
        raise
...
def find_similar_images(uploaded_image_features, color_histogram, directory, top_n=3):
    try:
        ...
        st.error(f"Error in finding similar images: {e}")
        raise
...
if uploaded_file is not None:
    ...
    try:
        ...
        st.write(f"Shape of uploaded_image_features: {uploaded_image_features.shape}")
